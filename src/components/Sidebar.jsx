...
export default function Sidebar({
  ...
  async function handlePhoto(e) {
    ...
    if (!file) return;
    setEditingPhoto(await readFileAsDataURL(file));
    e.target.value = "";
  }
    ...
    <>
      ...
      <aside className={`sidebar ${mobileOpen ? "open" : ""}`} style={{ padding: "24px 20px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
          <div style={{ position: "relative", width: 40, height: 40, flex: "none" }}>
            <label style={{ width: 40, height: 40, borderRadius: "50%", background: "#F97316", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 16, overflow: "hidden", cursor: "pointer" }}>
            ...
            {photo && (
              <button
                ...
                style={{ position: "absolute", right: -2, bottom: -2, width: 18, height: 18, borderRadius: "50%", background: "#F97316", border: "2px solid var(--white)", color: "white", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", padding: 0 }}
              >
                <svg viewBox="0 0 24 24" width="10" height="10" stroke="currentColor" strokeWidth="3" fill="none"><path d="M12 20h9"></path><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"></path></svg>
              </button>
            )}
          </div>
          <div>
        ...
        <nav className="sidebar-nav sidebar-nav-secondary">
          <button className="sidebar-link" onClick={() => { onOpenPlan(); onCloseMobile(); }} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", width: "100%", background: view === "documentos" ? "#1F242C" : "transparent", border: "none", cursor: "pointer", textAlign: "left" }}>
            <span style={{ display: "flex", color: view === "documentos" ? "white" : "#8B949E" }}><svg viewBox="0 0 24 24" width="18" height="18" stroke="currentColor" strokeWidth="2" fill="none"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg></span>
            <span style={{ fontSize: 13, color: view === "documentos" ? "white" : "#8B949E" }}>Documentos</span>
          </button>
      ...
      {editingPhoto && (
        <PhotoEditorModal
          ...
          onSave={(cropped) => { onPhotoChange(cropped); setEditingPhoto(null); }}
        />
      )}
    </>
  );
